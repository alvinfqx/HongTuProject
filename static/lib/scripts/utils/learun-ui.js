    ...
    function loadComboBoxView(json, searchValue, m) {
        if (json.length > 0) {
            ...
            if (options.defSelect) {
                ...
                /*if(typeof options.defSelect ==""){

                 }else{

                 }*/
                var len = $option.find('li').length;
                if (len > 2) {
                    $option.find('li').eq(1).click();//初始化下拉框默认选择第一个
                }
            }
        }
    }
...
$.fn.SetWebControls = function (data) {
    ...
    for (var key in data) {
        var id = $id.find('#' + key);
        if (id.attr('id')) {
            var type = id.attr('type') ? id.attr('type') : id.data('type');
...
/*
 ...
 无确定按钮开窗
 */
dialogOpenIsNoComfirmBtn = function (options) {
    Loading(true);
    var defaults = {
        id: null,
        title: '系统窗口',
...
dialogOpenATE = function (options) {
    Loading(true);
    var defaults = {
        id: null,
        title: '系统窗口',
...
dialogOpenCommit = function (options) {
    Loading(true);
    var defaults = {
        id: null,
        width: "100px",
...
dialogOpenContent = function (options) {
    Loading(true);
    var defaults = {
        id: null,
        width: "100px",
...
dialogOpenTest2 = function (options) {
    Loading(true);
    var defaults = {
        id: null,
        width: "100px",
...
dialogOpen4ATE = function (options) {
    Loading(true);
    var defaults = {
        id: null,
        width: "100px",
...
dialogContent4ATE = function (options) {
    var defaults = {
        id: null,
        width: "100px",
        height: "100px",
...
dialogIsAlert = function (content, type, callback) {
    if (type == -1) {
        type = 2;
    }
    top.layer.alert(content, {
...
request = function (keyValue) {
    ...
    // 正则筛选地址栏
    var reg = new RegExp("(^|&)" + keyValue + "=([^&]*)(&|$)");
    // 匹配目标参数
    var result = url.substr(1).match(reg);
    //返回参数值
    return result ? decodeURIComponent(result[2]) : null;
}
newrequest = function (keyValue) {
    var search = location.search.slice(1);
    var arr = search.split("&");
    for (var i = 0; i < arr.length; i++) {
        var ar = arr[i].split("=");
...
/*
 * Alvin --start
 * 子界面iframe 获取父界面iframe 的内容
 */
$.parentIframe = function () {
    if ($.isbrowsername() == "Chrome" || $.isbrowsername() == "FF") {
        return window.parent.document.getElementById(tabiframeId()).contentWindow;
    } else {
        return window.parent.document.getElementById(tabiframeId()).contentWindow
    }
}
 ...
 * 
 */
dialogOpenMax = function (options) {
    Loading(true);
    var defaults = {
        id: null,
        width: "100px",
