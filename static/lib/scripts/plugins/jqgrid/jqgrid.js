					...
					if(this.p.colNames.length !== this.p.colModel.length) alert(b.jgrid.errors.model);
						...
						var r = function(a, b) {
							...
							p = function(d, e, f, g, i, h) {
								...
								else if(k.cellattr && b.isFunction(k.cellattr) && (d = k.cellattr.call(a, i, f, g, k, h)) && "string" === typeof d)
									...
									if(c && S) {
										...
										for(; I < S;) {
											...
											if(k.repeatitems) {
												...
												for(C = 0; C < Z.length; C++) p = b.jgrid.getXmlData(w, Z[C]),
												...
												case "script":
													b.ajax(b.extend({
															...
															success: function(e, f, h) {
																...
																//table 加载错误弹框
																if(e && e.type == 3) {
																	dialogMsg('The table get data failed!', 2);
																}
																if(e && e.type == 4) {
																	dialogIsAlert('登录过期,请退出重新登录！', 0, function() {
															...
															error: function(d, e, f) {
																//alvin
																dialogMsg('The table loading error!', 2);
																//end
							...
							qa = function(c, e) {
								...
								"ltr" === f && (g += k);
								g += "</tr></tbody></table>";
								!0 === a.p.viewrecords && b("td#" + c + "_" + a.p.recordpos, "#" + l).append("<div dir='" + f + "' style='text-align:left' class='ui-paging-info'></div>");
						...
						for(g = 0; g < a.p.colModel.length; g++) a.p.colModel[g] = b.extend(!0, {},
							...
							a.p.colModel[g]), !1 === a.p.keyIndex && !0 === a.p.colModel[g].key && (a.p.keyIndex = g);
						...
						(function() {
							...
							!0 === a.p.shrinkToFit && 0 < g && (n = c.width - e * g - p, isNaN(a.p.height) || (n -= h, m = !0), d = 0, b.each(a.p.colModel,
								function(b) {
						...
						a.updatepager = function(c, d) {
							...
							if(n += a.p.toppager ? n ? "," + a.p.toppager : a.p.toppager : "") m = b.jgrid.formatter.integer || {},
								e = r(a.p.page),
								g = r(a.p.lastpage),
								b(".selbox", n)[this.p.useProp ? "prop" : "attr"]("disabled", !1), !0 === a.p.pginput && (b(".ui-pg-input", n).val(a.p.page), h = a.p.toppager ? "#sp_1" + p + ",#sp_1" + q : "#sp_1" + p, b(h).html(b.fmatter ? b.fmatter.util.NumberFormat(a.p.lastpage, m) : a.p.lastpage)),
		...
		getDataIDs: function() {
			...
			this.each(function() {
				if((c = this.rows.length) && 0 < c)
					for(; f < c;) b(this.rows[f]).hasClass("jqgrow") && (e[d] = this.rows[f].id, d++),
						f++
			});
			return e
		},
		...
		if(e.value) {
			...
			if(a.fmatter.isString(g)) {
				...
				for(m = 0; m < l.length; m++)
					if(f = l[m].split(b), 2 < f.length && (f[1] = a.map(f,
							function(a, b) {
								if(0 < b) return a
									...
									case "select":
										if(m = this.surl || g.dataUrl) a(l).append(n),
											...
											if(r) {
												...
												var v;
												if("string" === typeof r)
													for(m = r.split(u), v = 0; v < m.length; v++) r = m[v].split(w),
...
(function(a) {
	a.extend(a.jgrid, {
		...
		createModal: function(d, b, c, g, e, h, f) {
			...
			if(c.resize)
				if(a.fn.jqResize) a(k).append("<div class='jqResize ui-resizable-handle ui-resizable-se ui-icon ui-icon-gripsmall-diagonal-se'></div>"),
		...
		info_dialog: function(d, b, c, g) {
			...
			if(0 < e.buttons.length)
				for(g = 0; g < e.buttons.length; g++) void 0 === e.buttons[g].id && (e.buttons[g].id = "info_button_" + g),
				...
				case "select":
					...
					if(void 0 !== b.dataUrl) {
						...
						if("string" === typeof b.value)
							for(n = b.value.split(e), m = 0; m < n.length; m++) q = n[m].split(p),
		...
		checkDate: function(a, b) {
			...
			for(k = 0; k < a.length; k++) h = isNaN(b[k]) ? 0 : parseInt(b[k], 10),
				c[a[k]] = h,
				h = a[k], -1 !== h.indexOf("y") && (e = k), -1 !== h.indexOf("m") && (f = k), -1 !== h.indexOf("d") && (g = k);
			...
			if(!this.filter) {
				...
				if(q) {
					...
					this.createTableForGroup = function(g, l) {
						...
						if(void 0 !== g.groups)
							for(c = 0; c < g.groups.length; c++) e = a("<tr></tr>"),
								m.append(e),
					...
					this.createTableRowForRule = function(g, l) {
						...
						d = a("<td></td>");
						m.append(d);
						!0 === this.p.ruleButtons && (p = a("<input type='button' value='-' title='Delete rule' class='delete-rule ui-del'/>"), d.append(p), p.bind("click",
							function() {
					...
					this.toString = function() {
						function a(b) {
							var c = "(",
								f;
							if(void 0 !== b.groups)
								for(f = 0; f < b.groups.length; f++) 1 < c.length && (c = "OR" === b.groupOp ? c + " || " : c + " && "),
									c += a(b.groups[f]);
							if(void 0 !== b.rules)
								...
								complete: function(b, e) {
									...
									if("success" === e) {
										...
										if(!0 === g) {
											...
											a.isFunction(f.errorfunc) && f.errorfunc.call(d, c, b, e, null), !0 === f.restoreAfterError && a(d).jqGrid("restoreRow", c, f.afterrestorefunc)
									}
								},
			...
			return this.each(function() {
				...
				if(null === c || "object" !== typeof c && !d.isFunction(c)) this.p.grouping = !1;
					...
					for(b = 0; b < c.groupField.length; b++) c.groupOrder[b] || (c.groupOrder[b] = "asc"),
						...
						c.groupSummaryPos[b] || (c.groupSummaryPos[b] = "footer"), !0 === c.groupColumnShow[b] ? (c.visibiltyOnNextGrouping[b] = !0, d(this).jqGrid("showCol", c.groupField[b])) : (c.visibiltyOnNextGrouping[b] = d("#" + d.jgrid.jqID(this.p.id + "_" + c.groupField[b])).is(":visible"), d(this).jqGrid("hideCol", c.groupField[b]));
					c.summary = [];
			...
			this.each(function() {
				...
				if(f.hasClass(a)) {
					if(k.showSummaryOnHide) {
						if(e)
							for(; e && !(d(e).hasClass("jqfoot") && parseInt(d(e).attr("jqfootlevel"), 10) <= c);) d(e).hide(),
								e = e.nextSibling,
		...
		getNodeAncestors: function(b) {
			...
			this.each(function() {
				if(this.grid && this.p.treeGrid)
					for(var a = d(this).jqGrid("getNodeParent", b); a;) c.push(a),
		...
		addChildNode: function(b, c, a, h) {
			...
			if(a) {
				...
				if(void 0 === b || null === b) {
					s = e.p.data.length - 1;
					if(0 <= s)
						for(; 0 <= s;) l = Math.max(l, parseInt(e.p.data[s][e.p.localReader.id], 10)),
							s--;
					b = l + 1
				}
	...
	d.jgrid.extend({
		pivotSetup: function(q, n) {
			...
			this.each(function() {
				...
				if(c.colTotals)
					for(p = e.length; p--;)
						for(a = s; a < h.length; a++) y = h[a].name,
							v[y] = v[y] ? v[y] + parseFloat(e[p][y] || 0) : parseFloat(e[p][y] || 0);
				if(0 < x)
					for(a = 0; a < x; a++) b.groupingView.groupField[a] = h[a].name,
			...
			return this.each(function() {
				...
				if(c.grid && !c.p.treeGrid && b.fn.draggable && b.fn.droppable)
					if(void 0 === b("#jqgrid_dnd")[0] && b("body").append("<table id='jqgrid_dnd' class='ui-jqgrid-dnd'></table>"), "string" === typeof a && "updateDnD" === a && !0 === c.p.jqgdnd) d();
						...
						a || {}), a.connectWith)
					for(a.connectWith = a.connectWith.split(","), a.connectWith = b.map(a.connectWith,
							function(a) {
								return b.trim(a)
