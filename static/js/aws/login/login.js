	...
	$(function() {
		...
		$("#autologin").click(function() {
			if(!$(this).attr('checked')) {
				...
				top.$.cookie('learn_autologin', 1, {
					path: "/",
					expires: 7
				});
			} else {
				$(this).removeAttr("checked");
				top.$.cookie('learn_autologin', '', {
					path: "/",
					expires: -1
				});
		...
		$("#btnlogin").click(function() {
			...
			if($username.val() == "") {
				$username.focus();
				$(".alert-danger", $(".login-form")).html('请输入账户或手机号或邮箱。').show();

				return false;
			} else if($password.val() == "") {
				$password.focus();
				$(".alert-danger", $(".login-form")).html('请输入密码。').show();

				return false;
			} else if($verifycode.val() == "") {
				$verifycode.focus();
				$(".alert-danger", $(".login-form")).html('请输入验证码。').show();
	...
	this.CheckLogin = function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.type == 1) {
					if(top.$.cookie('learn_autologin') == 1) {
						top.$.cookie('learn_username', $.trim(username), {
							path: "/",
							expires: 7
						});
						top.$.cookie('learn_password', $.trim(password), {
							path: "/",
							expires: 7
						});
					}
