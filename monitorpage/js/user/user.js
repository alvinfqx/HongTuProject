...
//              isfilter:'true'
            ...
            $gridTable.jqGrid('setGridParam', {
                postData: { queryJson: JSON.stringify(queryJson), isfilter: 'true' }, page: 1
            }).trigger('reloadGrid');
        });
        //查询回车
    ...
    this.btn_add = function() {
        dialogOpen({
            id: "Form",
            title: '添加用户',
            url: '/monitorpage/view/user/user_flowForm.html',
            width: "750px",
            height: "550px",
    ...
    this.btn_edit = function() {
        ...
        if (checkedRow(keyValue)) {
            dialogOpen({
                id: "Form",
                title: '修改用户',
                url: '/monitorpage/view/user/user_flowForm.html?keyValue=' + keyValue,
                width: "750px",
                height: "550px",
                callBack: function (iframeId) {
                    top.frames[iframeId].model.AcceptClick();
    ...
    this.btn_revisepassword = function() {
        ...
        if (checkedRow(keyValue)) {
            dialogOpen({
                id: "RevisePassword",
                title: '重置密码',
                url: '/monitorpage/view/user/user_revisePassword.html?keyValue=' + keyValue + "&Account=" + escape(Account) + '&RealName=' + escape(RealName),
                width: "500px",
                height: "260px",
                callBack: function (iframeId) {
                    top.frames[iframeId].model.AcceptClick();
    ...
    this.btn_export = function() {
        location.href = cm.domain + "/BaseManage/User/ExportUserList";
    }
    //禁用
    ...
    this.btn_authorize = function() {
        ...
        if (checkedRow(keyValue)) {
            dialogOpen({
                id: "AllotAuthorize",
                title: '用户授权 - ' + RealName,
                url: '/monitorpage/view/user/user_allotAuthorize.html?userId=' + keyValue,
                width: "700px",
                height: "690px",
                btn: null
            });
    ...
    this.btn_ipfilter = function() {
        ...
        if (checkedRow(keyValue)) {
            dialogOpen({
                ...
                url: '/monitorpage/view/user/role_filterIP.html?objectId=' + keyValue + '&objectType=Uesr',
                width: "600px",
                height: "400px",
                btn: null
            });
    ...
    this.btn_timefilter = function() {
        ...
        if (checkedRow(keyValue)) {
            dialogOpen({
                ...
                url: '/monitorpage/view/user/role_filterTime.html?objectId=' + keyValue + '&objectType=Uesr',
                width: "640px",
                height: "480px",
                callBack: function (iframeId) {
                    top.frames[iframeId].model.AcceptClick();
