...
//				dialogClose();
	...
	this.AcceptClick = function() {
		...
		cm.ajax({
			//受理客户需求
			url: "/AfterSaleManage/aw_CustomerAssignedToNeed/SaveFormData?keyValue=" + keyValue, //回访
			type: "POST",
			dataType: "json",
			data: postData,
			success: function(data) {
	...
	this.show_msage = function(target) {
		...
		$("#returnVisitStaff").ComboBoxTreeSetValue("cfec7a42-669a-4d6e-a9cd-94ef19551dd0");//保存当前用户的方法
	...
	this.saveDataClick = function() {
		...
		cm.ajax({
			//受理客户需求
			url: "/AfterSaleManage/aw_CustomerAssignedToNeed/TempSaveFormData?keyValue=" + keyValue, //回访
			type: "POST",
			dataType: "json",
			data: postData,
			success: function(data) {
	...
	/** 转发本工单信息 **/
	this.SaveFormDataWithTempUserFeedBack = function() {
		var postData = $("#form").GetWebControls();
		Loading(true, "正在拼了命为您处理。。。");
		top.$("#loading_manage").attr('isTableLoading', 'true');
		postData["receiver"] = receiver_id_list.join(";");
		...
		cm.ajax({
			...
			success: function(data) {
				...
				dialogOpenCommitForward({
					id: 'Form',
					url: '/hongtupage/view/customerAssignedToNeed/FormWorkorderForward.html?slSeq=' + slseq,
					width: '1000px',
					height: '420px',
					callBack: function(iframeId) {
	...
	this.SaveFormDataWithMidWaySubmmit = function() { //中途提交
		var postData = $("#form").GetWebControls();
		Loading(true, "正在拼了命为您处理。。。");
		top.$("#loading_manage").attr('isTableLoading', 'true');
		cm.ajax({
			//受理客户需求
			url: "/AfterSaleManage/aw_AftersalesAssignedToNeed/SaveFormDataWithMidwaySubmit?keyValue=" + keyValue, //中途提交
			type: "POST",
			dataType: "json",
			data: postData,
			success: function(data) {
