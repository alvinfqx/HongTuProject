var viewModel = function() {
	...
	/** 保存本工单信息 **/
	this.SaveFormDataSaveClick = function() {
		var postData = $("#form").GetWebControls();
		Loading(true, "正在拼了命为您处理。。。");
		top.$("#loading_manage").attr('isTableLoading', 'true');
		cm.ajax({
	...
	//内部提交
	this.SaveFormDataWithInternalForwarding = function() {
		var postData = $("#form").GetWebControls();
		Loading(true, "正在拼了命为您处理。。。");
		top.$("#loading_manage").attr('isTableLoading', 'true');
		postData["receiver"] = receiver_id_list.join(";");
		...
		cm.ajax({
			...
			success: function(data) {
				...
				dialogOpenCommitForward({
					id: 'Form',
					url: '/hongtupage/view/customerAssignedToNeed/InternalForwarding.html?slSeq=' + slseq,
					width: '1000px',
					height: '420px',
					callBack: function(iframeId) {
						top.frames[iframeId].model.AcceptClick();
