...
//				dialogClose();
	...
	this.saveDataClick = function() {
		var json_data = {};
		json_data.aw_qc_processId = aw_qc_processIdid;
		json_data.workInfoId = keyValue;
		json_data.serviceType = $("#serviceType_s").attr('data-text'),
		json_data.accept_dept = $("#accept_dept").val();
		json_data.duty_dept = $("#duty_dept").val();
		json_data.relate_dept = $("#relate_dept").val();
		json_data.first_cause_conclusion = $("#first_cause_conclusion").val();
		json_data.confirm_cause_conclusion = $("#confirm_cause_conclusion").val();
		json_data.responsible_dept = $("#responsible_dept").val();
		json_data.cause_brief_confirm = $("#cause_brief_confir").attr('data-text'),
		json_data.processed_results = $("#processed_results").val();
		json_data.memo = $("#memo").val();
		var str = JSON.stringify(json_data);
		cm.ajax({
			//受理客户需求
			url: "/AfterSaleManage/aw_qc_process/SaveForm", //回访
			type: "POST",
			dataType: "json",
			data:{
				entity : str,
				keyValue : aw_qc_processIdid
			},
			success: function(data) {
				dialogMsg(data.message, 1);
				dialogClose();
