var viewModel = function() {
	...
	this.init = function() {
		...
		cm.ajax({
			//受理客户需求
			url: "/AfterSaleManage/productinfo/GetProductInfo", //回访
			type: "POST",
			dataType: "json",
			data: {
	...
	this.btn_background = function() { //更换背景图
		dialogOpen({
			id: 'Form',
			title: '更换背景图',
			url: '/hongtupage/view/equipment/replace.html?keyValue=' + keyValue,
			width: "750px",
			height: "500px",
	...
	this.initPoint = function() {
		cm.ajax({
			url: '/AfterSaleManage/aw_flowchart/GetProductSettingList',
			type: 'GET',
			data: {
				productInfoId: keyValue
			},
	...
	$('body').on('click', '.delete_class', function() {
		...
		if(!!id) {
			dialogConfirm('确定要删除该节点吗？', function(isTrue) {
				if(isTrue) {
					cm.ajax({
						url: '/AfterSaleManage/aw_flowchart/ProductSettingRemove',
						type: 'POST',
						data: {
							keyValue: id
						},
	...
	$('body').on('dblclick', '.drag_div', function() {
		...
		dialogOpen({
			id: 'CollectionPoint',
			title: '选择采集点',
			url: '/hongtupage/view/equipment/monitor_col_point.html?keyValue=' + keyValue,
			width: '750px',
			height: '500px',
			btn: ['确定', '取消'],
			callBack: function(iframeId) {
				top.frames[iframeId].model.AcceptClick(function(data) {
					...
					html += '</a>';
					html += data.NODE_NAME;
					html += '<input type="hidden" value="' + data.product_NODE_ID + '"/>';
					$obj.html(html);
				});
			}
		});
	...
	this.btn_save = function() {
		...
		cm.ajax({
			url: '/AfterSaleManage/aw_flowchart/ProductSettingSave',
			type: 'POST',
			data: {
