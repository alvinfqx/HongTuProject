var viewModel = function() {
	...
	//加载树
	var _itemId = '';
	this.GetTree = function(paramStr) {//传参到查询
		var item = {
...
//		})
	...
	this.GetGrid = function() {
		...
		$gridTable.jqGrid({
			url: "/BaseManage/User/GetPageListOrganize?isfilter=",
			datatype: "json",
			postData: {
...
//					ProvinceId: _itemId,
	...
	this.btn_add = function() {
			...
			id: "Form",
			title: '添加用户',
			url: '/hongtupage/view/user/user_flowForm.html?_itemId='+_itemId+'&keyValue='+'',
			width: "750px",
			height: "550px",
	...
	this.btn_edit = function() {
		...
		if(checkedRow(keyValue)) {
			dialogOpen({
				id: "Form",
				title: '修改用户',
				url: '/hongtupage/view/user/user_flowForm.html?keyValue=' + keyValue+'&_itemId='+_itemId,
				width: "750px",
				height: "550px",
				callBack: function(iframeId) {
					top.frames[iframeId].model.AcceptClick();
	...
	this.btn_revisepassword = function() {
		...
		if(checkedRow(keyValue)) {
			dialogOpen({
				id: "RevisePassword",
				title: '重置密码',
				url: '/hongtupage/view/user/user_revisePassword.html?keyValue=' + keyValue + "&Account=" + escape(Account) + '&RealName=' + escape(RealName),
				width: "500px",
				height: "260px",
				callBack: function(iframeId) {
					top.frames[iframeId].model.AcceptClick();
	...
	this.btn_authorize = function() {
		...
		if(checkedRow(keyValue)) {
			dialogOpen({
				id: "AllotAuthorize",
				title: '用户授权 - ' + RealName,
				url: '/hongtupage/view/user/user_allotAuthorize.html?userId=' + keyValue,
				width: "700px",
				height: "690px",
				btn: null
			});
	...
	this.btn_ipfilter = function() {
		...
		if(checkedRow(keyValue)) {
			dialogOpen({
				...
				url: '/hongtupage/view/user/role_filterIP.html?objectId=' + keyValue + '&objectType=Uesr',
				width: "600px",
				height: "400px",
				btn: null
			});
	...
	this.btn_timefilter = function() {
		...
		if(checkedRow(keyValue)) {
			dialogOpen({
				...
				url: '/hongtupage/view/user/role_filterTime.html?objectId=' + keyValue + '&objectType=Uesr',
				width: "640px",
				height: "480px",
				callBack: function(iframeId) {
					top.frames[iframeId].model.AcceptClick();
