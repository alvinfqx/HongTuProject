var viewModel = function() {
	...
	this.btn_edit = function() {
		...
		var entry_type = $('#gridTable').jqGridRowValue('entry_type');
		var userId = sessionStorage.getItem("userId");
//		alert(userId);
		if(checkedRow(keyValue)) {
			if(workstatus == "制定方案") {
				if(entry_type != "1" && entry_type != "2") { //跳新页面
					if(userId!=entry_type){
						dialogOpenCommitForward({
						id: 'Form',
						url: '/hongtupage/view/customerAssignedToNeed/Form_entry_type.html?keyValue=' + keyValue + '&slSeq=' + slSeq,
						width: 'px',
						height: 'px',
						cancel: function() {
						//删除穿梭框session
						...
						callBack: function(iframeId) {
							top.frames[iframeId].model.SubmissionAcceptClick();//提交
						},
						callBack2: function(iframeId) {
							top.frames[iframeId].model.ForwardAcceptClick();//转发
						}
					})
					}else{
						dialogOpenCommitATE({
						...
						callBack3: function(iframeId) {
							top.frames[iframeId].model.SaveFormDataWithInternalForwarding();
