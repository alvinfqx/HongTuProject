var viewModel = function() {
	...
	$(function() {
		...
		//alvin star
		self.initGridTable();
		self.getTableData(_itemId, productInfoId);
		//alvin end
	});
	...
	$('#tab li').click(function() {
		if(!productInfoId || !productCatId) {
			dialogMsg('请先保存基本信息配置数据！', 0);
			return false;
		}
		//点击索引
	 ...
	 */
	//点击保存后，初始化数据
	this.initGetData = function(value) {
		cm.ajax({
			...
			success: function(res) {
				...
				$("#productCatId").ComboBoxTreeSetValue(data.productcatid);
...
//					var data_img_url = data.photoUrl + '&token=' + cm.token;
	...
	//初始化产品介绍
	this.productHtml = function(value) {
		cm.ajax({
	...
	$("#ext_pro_id").on("click", ".lr-remove", function() {
		...
		var trs = $(this).parentsUntil("tbody");
		var tbod = $(this).parentsUntil("table");

		if(tbod.children("tr").length == 1) {
			dialogMsg("不能再删除了哟！", 0);
			return false;
		}
		trs.remove();
	...
	this.preservation_b = function() {
		...
		cm.ajax({
			...
			success: function(data) {
				...
				//保存成功后清空删除数组
				delete_arr = [];
				productInfoId = data.resultdata.productInfoId;
				productCatId = data.resultdata.productCatId;
				//初始化BOM树
				self.GetTree();
				//初始化产品基本资料
				self.initGetData(productInfoId);
				//初始化产品介绍
				self.productHtml(productInfoId);
				//初始化拓展属性
				self.initAttr(productInfoId);
				//提示
	...
	//获取规格参数数据
	this.getSpecifData = function(val) {
		cm.ajax({
	...
	//赋值model2数据
	this.load_model2_data = function(val) {
		var that = this;
		//再次初始化获取model2的值
	...
	this.save_specif_one = function() {
		...
		var postSpeData = {};
		postSpeData.keyValue = '';
		postSpeData.productInfoId = productInfoId;
		//循环获取新增的table
		var detailJson = [];
		...
		cm.ajax({
			...
			success: function(data) {
				if(data.type && data.type == 1) {
                    self.getSpecifData(productInfoId);   
					self.load_model2_data(productInfoId);
					dialogMsg(data.message, 1);
				} else {
					dialogMsg(data.message, 2);
				}
	...
	//规格二 行内新增
	//规格配置--新增方法--表格行内新增
	this.addSpecifTwoRow = function($grid, row, i) {
		var html = '';
		html += '<input name="cfgItemId" type="hidden" class="editable center" value="' + (row.cfgItemId || "") + '"/>';
		html += '<input name="cfigItemCatgId" type="hidden" class="editable center" value="' + (row.cfigItemCatgId || "") + '"/>';
		html += '<input checkexpession="NotNull" isvalid="yes" name="cfgItemName" type="text"  class="editable center" value="' + (row.cfgItemName || "") + '"/>';
		var rowdata = {
			//主键
			cfgItemId: row.cfgItemId,
			cfigItemCatgId: row.cfigItemCatgId,
			productInfoId: row.productInfoId,
			//字段
			cfgItemName: html,
			cfgItemValue: '<input name="cfgItemValue" type="text" class="editable center" value="' + (row.cfgItemValue || "") + '"/>',
		}
		$grid.jqGrid('addRowData', i, rowdata);
	};
	...
	$("#content_id").on('click', '.add_row_spe', function() {
		...
		self.addSpecifTwoRow($gridTwoSpefic, {}, len);
	...
	this.save_specif_two = function() {
		var postSpeTwoData = {};
		postSpeTwoData.keyValue = '';
		postSpeTwoData.productInfoId = productInfoId;
	...
	this.btn_logistics = function() {
		dialogOpen({
			id: "DataItemSort",
			title: '物料类型字典',
			url: '/hongtupage/view/product_attributes/logistics.html',
			width: "800px",
			height: "500px",
			btn: null
		});
	...
	//根据产品id获取产品的售后服务
	this.getAfterSaleData = function() {
		cm.ajax({
	...
	//根据产品id获取质保期的html介绍
	this.GetWarrantyPeriodData = function() {
		cm.ajax({
